...
stat = [[i.strip().split("\t") for i in open("data/"+ime).readlines()] for ime in tekme]
stat = [[[n for n in j] for j in i] for i in stat]
#Sstat = [tostr(i).contains("-"): [j for j in tostr(i).split("-")] for i in stat]
for i in stat:
    for j in i:
        print j   
    print "\n"
